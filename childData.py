...
def AttendByDay():
    ...
    conn = sqlite3.connect('data.db')
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS Attendence (Aid Integer primary key, Cid INTEGER FOREIGN KEY NOT NULL REFERENCES data(id), date text, enter text, leave text, Cname TEXT FOREIGN KEY NOT NULL REFERENCES data(name);')
